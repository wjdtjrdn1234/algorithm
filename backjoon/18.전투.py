...
def dfs(y,x):
    ...
    for i in range(4):
        ny = y+dy[i]
        nx = x+dx[i]
        if 0<=ny<m and 0<=nx<n and visit[ny][nx]==False and graph[ny][nx]=='W':
            dfs(ny,nx)
...
def dfs2(y,x):
    ...
    for i in range(4):
        ny = y+dy[i]
        nx = x+dx[i]
        if 0<=ny<m and 0<=nx<n and visit[ny][nx]==False and graph[ny][nx]=="B":
            dfs2(ny,nx)
# def bfs(s):
#     q = deque()
#     q.append(s)
...
for y in range(m):
    for x in range(n):
        if graph[y][x]=='W' and visit[y][x]==False:
           our =0
           dfs(y, x)
           our_sum+=our*our
...
for y in range(m):
    for x in range(n):
        if graph[y][x] == 'B' and visit[y][x] == False:
            your = 0
            dfs2(y, x)
            your_sum +=your*your

print(our_sum, your_sum)
