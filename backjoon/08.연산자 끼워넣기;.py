...
maximum = -1e9 #10억보다 크거나 같고, 10억보다 작거나 같은 결과
minimum = 1e9


def dfs(depth, total, plus, minus, multiply, divide):
    global maximum, minimum
    if depth == N: #모든 수를 다 탐색했으면
...
dfs(1, num[0], op[0], op[1], op[2], op[3]) #현재탐색하는 수의 인덱스,시작값,각 연산갯수(맨 처음수는 탐색할필요x)
print(maximum)
print(minimum)

# 백트래킹 DFS: 연산자의 갯수만큼 탐색을하며, 연산자가존재하면
# 그 연산을 수행하며 재귀호출을 통해 탐색을 진행한다.
