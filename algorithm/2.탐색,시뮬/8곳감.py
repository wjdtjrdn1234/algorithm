...
for i in range(m):
    ...
    if t==0:
        for _ in range(k):  #왼쪽방향으로 회전 : 제일 앞에거를 꺼내서 제일 뒤에다 삽입
            arr[h-1].append(arr[h-1].pop(0))
    else:
        for _ in range(k):  #오른쪽방향으로 회전: 제일 뒤에거를 꺼내서 제일 앞에다 삽입
...
for i in range(n):
    for j in range(s,e+1):
        result+=arr[i][j]
    if i<n//2: #행이 절반이하 라면, s,e는 좁혀들어감
        s+=1
        e-=1
    else: #s,e는 넓혀들어감
        s-=1
