...
# index 번호를 가르키는 변수2개를 잡음(lt,rt)
#lt~rt핪이 m값과 비교
#적으면 rt증가
#같으면 count수 증가하고 lt가 가르키는값을 뺴주고 lt를 증가시키고 또 비교
#크면 lt가 가르키는값을 뺴주고 lt를 증가시키고 또 비교
#rt가 n보다 커지면 break
...
#sort() = nlogn -> 이미정렬된것을 활용하면 n번만에 가능
#정답
# n= int(input())
