...
res = ''
for x in n:
    if x.isdecimal(): #10진수인지확인
        res+=x
    else:
        if x=='(':
            stack.append(x)
        elif x=='*' or x=='/':
            while stack and (stack[-1]=='*' or stack[-1]=='/'): #스택에있는 연산자가 우선순위가 같은거 다뺴버리기
                res+=stack.pop()
            stack.append(x)
        elif x == '+' or x == '-': #괄호안에 연산자
            while stack and stack[-1]!='(': #여는괄호 아니라면 스택연산자 모두 pop(+,- 우선순위가 제일 낮으므로)
                res+=stack.pop()
            stack.append(x)
        elif x== ')':
            while stack and stack[-1] != '(': #여는괄호 만날떄까지 연산자 모두 pop
                res += stack.pop()
            stack.pop()
while stack: #스택이 남아있는경우
    res+=stack.pop()
print(res)


#같은 우선순위에 있더라도 스택에 있는 연산자가 우선임
#스택에 있는 연산자와 비교해 같거나 빠르면 pop,아니면 그위에 append
