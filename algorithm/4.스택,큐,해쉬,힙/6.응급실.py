...
arr = [(pos,value) for pos,value in enumerate(list(map(int,input().split())))] #입력 list의 순서,값 튜플로저장
#[(0,60),(1,50),(2,40)] // [60,50,40]
arr= deque(arr)
cnt=0

while True:
    cur=arr.popleft()
    if any(cur[1]<x[1] for x in arr):#arr모든 값 접근해서 조건확인 -> 단 한개라도 참이면 참
        arr.append(cur)
    else:
        cnt+=1
        if cur[0]==m:
