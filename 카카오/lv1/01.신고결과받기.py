...
def solution(id_list, report, k):
    ...
    report = set(report)
    user_report_who = defaultdict(set)
    nums_reported = defaultdict(int)
    suspend=[]
    for r in report:
        user_report , user_reported = r.split()
        user_report_who[user_report].add(user_reported)
        nums_reported[user_reported]+=1
        if nums_reported[user_reported]==k:
            suspend.append(user_reported)
    for reported in suspend:
        for i in range(len(id_list)):
            if reported in user_report_who[id_list[i]]:
                answer[i]+=1
    return answer
