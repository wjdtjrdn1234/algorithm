...
def solution(rows, columns, queries):
    ...
    for x1, y1, x2, y2 in queries:
        ...
        for m in range(x1 - 1, x2 - 1):
            array[m][y1 - 1] = array[m + 1][y1 - 1]
            mini = min(mini, array[m + 1][y1 - 1])

        for j in range(y1 - 1, y2 - 1):
            array[x2 - 1][j] = array[x2 - 1][j + 1]
            mini = min(mini, array[x2 - 1][j + 1])

        for k in range(x2 - 1, x1 - 1, -1):
            array[k][y2 - 1] = array[k - 1][y2 - 1]
            mini = min(mini, array[k - 1][y2 - 1])

        for i in range(y2 - 1, y1 - 1, -1):
            array[x1 - 1][i] = array[x1 - 1][i - 1]
            mini = min(mini, array[x1 - 1][i - 1])
