...
def DFS(L, s):
    ...
    if L == m:
        ...
        for i in range(s, len(pz)):
            cb[L] = i #cb는 피자집의 인덱스번호
            DFS(L + 1, i + 1) #가지는 선택한값보다 큰값이상으로 뻣음
if __name__ == "__main__":
    n, m = map(int, input().split())
    board = [list(map(int, input().split())) for _ in range(n)]
    hs = []
    pz = []
    cb = [0] * m  #조합의 경우의수 //즉 살아남은 피자집
    res = 2147000000
    for i in range(n):
        for j in range(n):
            if board[i][j] == 1:
...
#6개의 피자집에서 4개의 피자집을 고르는 6C4 조합
#각 레벨에서 6->5->4->3 이런식으로 뻣어나감
