...
min=float('inf') #파이썬에서 가장큰값

for idx ,value in enumerate(arr):
    tmp = abs(value-average)
    if tmp<min:
        min = tmp #평균거리-내거리
        score = value #학생 점수
        order = idx+1 #학생 번호
    elif tmp==min and value>score: #같은거리가 나왔을떄 점수가 큰사람이 정답  # 현재점수가 그전에 점수보다 크다면 // 높은점수가 여러개일경우는 고려안해도됨//어짜피 if문을 안도니까
            score=value
            order=idx+1
...
#거리(평균-내점수),점수,순서 3가지를 비교해서 최솟값 찾는문제
#round,0.5 => 반올림
#abs => 절대값
#sum => 배열 원소합
...
#     if abs(i[1]-average)<arrMin:
#         arrMin=abs(i[1] - average)
#         min = []
#         min.append((i[0],i[1]))
#     elif abs(i[1]-average)==arrMin:
#         min.append((i[0],i[1]))
#
#
# if len(min) == 1:
