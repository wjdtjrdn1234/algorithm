...
while lt<=rt:
    ...
    if count(mid)>=n:
        res=mid
        lt=mid+1 #더 좋은답을 찾기위해 범위를 줄여줌
    else:
        rt=mid-1 #조건이 만족안된다면 범위가 너무길다는소리
print(res)
...
#결정알고리즘: 답이 특정 범위안에서 유효한지 안한지 확인 ->범위좁힘(절반을날림)->이 답보다 더 최선의 답이있는지 계속 좁혀감
#범위는 1~802 (랜선중 최장길이)-> 중간값구해서 답이되는지 확인 (랜선의 몫을 구해서 n과 같은지 비교)
# 구한 값이 n보다 작으면 중간값보다 큰값은 버리고, 다시 중간값을 구함
# 구한 값이 n보다 크면 중간값보다 작은 버리고, 다시 중간값을 구함
#만약 조건에 만족하면 res에 결과값을 넣고 ,최적의 해가 있는지 다시탐색
...
#이분검색은 lt(맨왼쪽), rt(맨오른쪽) 를 변수를 설정하고
#mid (lt+rt //2 )라는 중간변수를 만들어
# arr[mid] 와 m과 비교해 ,
# arr[mid]가 크면 , 오른쪽 범위를 버린다 -> rt = mid-1로 바꾼다
# arr[mid]가 작으면 , 왼쪽 범위를 버린다 -> lt = mid+1로 바꾼다
# lt가 rt보다 커지면 탐색종료
# 수행시간 o(logn)
