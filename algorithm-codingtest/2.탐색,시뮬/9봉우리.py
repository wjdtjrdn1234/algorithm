...
arr.insert(0,[0]*n) #0행에 n개의 0삽입
arr.append([0]*n) #n+1행에 n개의 0삽입

for x in arr: #각각의 행에 앞,끝에 0삽입
    x.insert(0,0)
    x.append(0)
...
dx = [-1,0,1,0]
dy = [0,1,0,-1]
for i in range(1,n+1): #가장자리 제외하고
    for j in range(1,n+1):
        if all(arr[i][j]>arr[i+dx[k]][j+dy[k]] for k in range(4)):#봉우리인지확인:해당조건이 모두참이면 true
            count+=1
print(count)
